...
const ESTAGIOS: Estagio[] = ['Startup', 'Scale-up', 'Corporação']

function toggle<T>(list: T[], value: T): T[] {
  return list.includes(value) ? list.filter((v) => v !== value) : [...list, value]
}

export default function EmpresasInstaladas() {
  ...
  const filterGroups = (
    <>
      <SidebarFilter label="Setor" options={SETORES} active={setores} onChange={(v) => setSetores((prev) => toggle(prev, v as Setor))} counts={contagens.setor} theme="dark" />
      <SidebarFilter label="Estágio" options={ESTAGIOS} active={estagios} onChange={(v) => setEstagios((prev) => toggle(prev, v as Estagio))} counts={contagens.estagio} theme="dark" />
    </>
  )
  ...
  return (
    <Section padding="md" className="bg-brand-navy">
            ...
            {filtered.length > 0 ? (
              <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                {filtered.map((empresa) => {
                  const details = EMPRESA_DETAILS[empresa.slug]
                  return (
                    <button
                      key={empresa.slug}
                      type="button"
      ...
      {/* Modal */}
      {selected && (() => {
        const details = EMPRESA_DETAILS[selected.slug]
        return (
          <div
          ...
          >
            ...
            <div className="relative w-full max-w-5xl my-auto rounded-3xl border border-white/10 bg-white shadow-2xl overflow-hidden">
              ...
              <div className="grid md:grid-cols-2">
                ...
                <div className="relative min-h-[320px] md:min-h-[420px] bg-neutral-100">
                  <Image
                    ...
                    alt={`${details.founder.name}, ${details.founder.title} de ${selected.nome}`}
                    fill
              ...
              <div className="border-t border-neutral-200 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-px bg-neutral-200">
                {[
                  { label: details.founder.title, value: details.founder.name },
                  { label: 'Investimento inicial', value: `${details.investment.round}, ${details.investment.year}` },
                  { label: 'Status', value: details.status, color: ESTAGIO_CONFIG[selected.estagio].text },
