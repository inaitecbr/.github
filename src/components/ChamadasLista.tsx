...
const ENTRADA_KEYS: EntradaKey[] = ['edital', 'inscricao-continua', 'parceria', 'convite']
const PRAZOS: Prazo[] = ['urgente', 'mes', 'continuo']

const PUBLICO_LABELS: Record<PublicoKey, string> = {
  startups: 'Startups',
  empresas: 'Empresas',
...
const PUBLICO_LABEL_FULL: Record<PublicoKey, string> = {
...
const ENTRADA_LABELS: Record<EntradaKey, string> = {
  edital: 'Edital',
...
export default function ChamadasLista({ programas: programasProp }: Props) {
  ...
  const filtered = useMemo(() => {
    return programas
      .filter((p) => {
        if (publicos.length > 0 && (!p.publicoKey || !publicos.includes(p.publicoKey))) return false
        if (entradas.length > 0 && (!p.entradaKey || !entradas.includes(p.entradaKey))) return false
  ...
  }, [programas, publicos, entradas, prazos, endOfMonth])
  ...
  const filtrosAtivos = publicos.length + entradas.length + prazos.length

  const contagens = useMemo(() => {
    const base =
      publicos.length === 0 ? programas : programas.filter((p) => p.publicoKey && publicos.includes(p.publicoKey))

    const publico: Record<string, number> = {}
    programas.forEach((p) => {
      if (p.publicoKey) publico[p.publicoKey] = (publico[p.publicoKey] || 0) + 1
    })

    const entrada: Record<string, number> = {}
    base.forEach((p) => {
      if (p.entradaKey) entrada[p.entradaKey] = (entrada[p.entradaKey] || 0) + 1
    })

    const prazo: Record<string, number> = {}
    PRAZOS.forEach((pr) => {
      prazo[pr] = base.filter((p) => matchesPrazo(p.deadline, pr, endOfMonth)).length
    })

    return { publico, entrada, prazo }
  }, [programas, publicos, endOfMonth])
        ...
        <aside className="lg:sticky lg:top-[88px]">
          ...
          <details className="lg:hidden rounded-2xl bg-white border border-border overflow-hidden group">
            ...
            <div className="px-5 pb-5 pt-1 flex flex-col border-t border-border">
              <SidebarFilter label="Para" options={PUBLICO_KEYS} active={publicos} onChange={(v) => setPublicos((prev) => toggle(prev, v as PublicoKey))} counts={contagens.publico} labels={PUBLICO_LABELS} />
              <SidebarFilter label="Tipo" options={ENTRADA_KEYS} active={entradas} onChange={(v) => setEntradas((prev) => toggle(prev, v as EntradaKey))} counts={contagens.entrada} labels={ENTRADA_LABELS} />
        ...
        <div>
          ...
          {filtered.length > 0 ? (
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5 items-start">
              {filtered.map((p) => {
                const accentPublico = p.publicoKey ? PUBLICO_COLORS[p.publicoKey] : 'var(--color-brand-orange)'
                  ...
                  >
                    <div className="flex flex-col flex-1 p-7">
                      ...
                      {p.publicoKey && (
                        ...
                        >
                          {PUBLICO_LABEL_FULL[p.publicoKey]}
                        </p>
                      )}
            ...
            <div className="rounded-2xl border border-dashed border-brand-navy/15 bg-white/50 p-12 text-center">
              ...
              <p className="text-sm text-brand-navy/60 mb-6 max-w-md mx-auto">
                Ajuste algum filtro ou fale com a gente — montamos uma jornada sob medida pra sua empresa.
              </p>
