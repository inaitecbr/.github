...
// ── Configuração técnica (não traduzível): accent / orb / href / icon ──
type PillarAsset = {
  accent: string
  orbPos: string
  href: string
...
export default function EcosystemAccordion() {
  ...
  const pillarsContent = t.raw('pillars') as PillarContent[]
  const pillars = pillarsContent.map((p, i) => ({ ...p, ...PILLAR_ASSETS[i] }))
  ...
  return (
    <section className="relative pt-16 pb-0 md:pb-16 overflow-hidden">
      {/* ── Feijinhos orgânicos de fundo — efeito de imersão ── */}
      <div
      ...
      <Container className="relative">
        ...
        {/* ── Accordion horizontal (desktop) / Cards full-screen (mobile) ── */}
              ...
              >
                ...
                <div
                  className={[
                    ...
                    isOpen ? 'ring-1 ring-white/10' : 'ring-1 ring-white/[0.06] group-hover:ring-white/20',
                  ].join(' ')}
                />
                ...
                {/* ═══ ESTADO ABERTO — narrativa (topo) → métrica (rodapé) ═══ */}
                <div
                  className={[
                ...
                >
                  {/* TOPO — eyebrow + título + descrição + tags */}
                  <div>
                    <div className="flex flex-col items-start gap-4 mb-6 md:flex-row md:items-start md:justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="text-[10px] font-bold uppercase tracking-[0.25em]" style={{ color: p.accent }}>
                            {p.subtitle}
                          </div>
                      ...
                      <div className="inline-flex items-center gap-2 rounded-full bg-white/5 border border-white/10 px-3 py-1.5 backdrop-blur-sm">
                        <span className="relative flex w-1.5 h-1.5">
                          <span
                            ...
                            style={{ backgroundColor: p.accent }}
                          />
                          <span className="relative rounded-full w-1.5 h-1.5" style={{ backgroundColor: p.accent }} />
                        </span>
                    ...
                    <p className="text-white/65 text-[14px] leading-relaxed max-w-lg">
                      {p.desc}
                    </p>
                  ...
                  {/* RODAPÉ — métrica grande + CTA específico */}
        ...
        {/* ── Barra de progresso (desktop only — mobile mostra todos abertos) ── */}
